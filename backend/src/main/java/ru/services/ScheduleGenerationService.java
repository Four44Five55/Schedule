...
public class ScheduleGenerationService {
     ...
     *
     * <h4>Период — не фильтр «для удобства», а условие правильности</h4>
     * <p>Кандидат ищется среди сессий <b>своего</b> периода. Иначе самой свежей запросто окажется
     * импортная (импорт создаёт новую на каждый прогон), и экран другого периода получит чужую
     * сессию: находки по аудиториям посчитаются по чужому расписанию, а перегенерация, замок и
     * перенос уйдут туда же.</p>
     *
     ...
     *
     * @param periodId учебный период экрана; {@code null} — период не выбран, и берётся самая
     *                 свежая сессия с размещениями
     * @param user     пользователь, выполняющий действие
