...
/**
 * Подпись преподавателя с регалиями: {@code «п-к юст Иванов И.И., ктн, доц»}.
 *
 ...
 * склейки.</p>
 *
 * <p><b>Точек нет нигде</b> (решение заказчика, уточнено 2026-08-14). Сокращения хранятся без
 * точек — «п-к», «юст», «доц», «т» — и <b>собираются тоже без них</b>: степень это «ктн», а не
 * «к.т.н.». Причина не в вкусе: ровно так пишут документы, из которых мы читаем данные (в выгрузке
 * сторонней программы — «ктн», «кфмн», «двн»). Пока конвенции ввода и вывода различались, наша
 * подпись и подпись из файла не сравнивались напрямую, и расхождение мог заметить только человек.</p>
 */
public final class EducatorTitles {
     ...
     * @param degreeShort  буквенная часть степени: «к» (в «ктн»)
    ...
    /**
     * Составное сокращение степени: «ктн», «кфмн», «двн».
     *
     * <p><b>Слитно и без точек</b> (решение заказчика 2026-08-14, было «к.т.н.»). Так пишут
     * документы, из которых мы читаем данные: в выгрузке сторонней программы стоит «ктн», «кфмн»,
     * «двн». Одна конвенция на ввод и вывод означает, что подпись из файла и наша подпись
     * <b>сравнимы посимвольно</b> — расхождение видно сверкой, а не глазами.</p>
     *
     * <p>Отрасль без уровня — не степень (одни «технические» ничего не значат), поэтому в таком
     * случае пусто. Уровень без отрасли даёт «канд наук»: записи «кн» не существует.</p>
     */
