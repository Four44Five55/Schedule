 ...
 * («Тополев В.Ф. двн проф; п/п-к Астахов С.В.»), шапку преподавательского файла («к-н Ветров Р.И.
 * ктн») и <b>имя файла</b> («ВетровР.И.» — без пробела, «старая реализация»). Последнее выходит
...
public final class EducatorNameDecoder {
     ...
     * («ВетровР.И.»), на которых сломалась бы любая эвристика «где кончается фамилия».</p>
     *
    ...
    public record EducatorName(
        ...
        /**
         * Ключ сопоставления с нашим {@code educator.name}: «Астахов С.В.».
         *
     ...
     * Разбирает подпись.
     *
     * @param raw            подпись из файла: «п/п-к Астахов С.В.», «Тополев В.Ф. двн проф», «ВетровР.И.»
