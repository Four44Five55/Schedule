 ...
 *
 * <p>В отличие от корпуса или аудитории, здесь каскада нет <b>намеренно</b>: все три ссылки на
 * подразделение ({@code org_unit.parent_id}, {@code educator.org_unit_id},
 * {@code groups.org_unit_id}) заведены с {@code ON DELETE RESTRICT}. Расформирование факультета
 * не должно молча уносить кафедры, а кафедры — преподавателей: люди и группы не производны от
 * подразделения и обязаны его пережить. Поэтому подразделение со связями удалить нельзя вовсе —
 * сначала перепривязать или поднять на уровень выше.</p>
 *
