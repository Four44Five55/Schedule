...
public class ScheduleCommandController {
     ...
     * самая свежая сессия по всем периодам, а это неверно: импорт создаёт новую сессию на каждый
     * прогон, и экран любого периода получил бы её. Параметр необязателен ради легаси-вызовов;
     * правило поиска — в {@code getOrCreateEditableSession}.</p>
     *
