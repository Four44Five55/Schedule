...
CREATE TABLE discipline_curriculum (
                                       id            SERIAL PRIMARY KEY,
                                       discipline_id BIGINT NOT NULL REFERENCES discipline (id),
                                       start_slot_id BIGINT REFERENCES curriculum_slot (id),
                                       end_slot_id   BIGINT REFERENCES curriculum_slot (id)
);

CREATE TABLE slot_chain (
                            id            SERIAL PRIMARY KEY,
                            discipline_id BIGINT NOT NULL REFERENCES discipline (id),
                            slot_a_id     BIGINT NOT NULL REFERENCES curriculum_slot (id),
                            slot_b_id     BIGINT NOT NULL REFERENCES curriculum_slot (id),
...
WITH study_types AS (
    ...
    UNION SELECT 2, 'PRACTICAL_WORK', 'Практическое занятие'
    UNION SELECT 3, 'LAB_WORK', 'Лабораторная работа'
    UNION SELECT 4, 'SEMINAR', 'Семинар'
    UNION SELECT 5, 'GROUP_WORK', 'Групповое занятие'
    UNION SELECT 6, 'GROUP_EXERCISE', 'Групповое упражнение'
    UNION SELECT 7, 'QUIZ', 'Контрольная работа'
    UNION SELECT 8, 'INDIVIDUAL_REVIEW_INTERVIEW', 'Индивидуальное контрольное собеседование'
    UNION SELECT 9, 'CREDIT_WITH_GRADE', 'Зачет с оценкой'
    UNION SELECT 10, 'CREDIT_WITHOUT_GRADE', 'Зачет без оценки'
    UNION SELECT 11, 'EXAM', 'Экзамен'
    UNION SELECT 12, 'INDEPENDENT_STUDY', 'Самостоятельная работа'
),
     type_counts AS (
         SELECT
             st.sort_order,
