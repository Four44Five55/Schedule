...
-- Справочник назначений аудиторий
CREATE TABLE auditorium_purpose
(
    id   SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL UNIQUE -- "Лекционная", "Семинарская", "Компьютерный класс", "Химическая лаборатория"
);
-- Справочник всего возможного оснащения
CREATE TABLE feature
(
    id   SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL UNIQUE, -- "Проектор", "Интерактивная доска", "Компьютеры (30 шт.)"
    code VARCHAR(50)  NOT NULL UNIQUE  -- Короткий код, например, 'PROJECTOR', 'PC_30'
);
...
(
    ...
    purpose_id  INTEGER REFERENCES auditorium_purpose (id),
...
-- =======================================================================
--  "Оснащение" (Features / Equipment) возможного оснащения аудиторий
...
(
    ...
    PRIMARY KEY (auditorium_id, feature_id)
);
...
(
    ...
    required_purpose_id    INTEGER REFERENCES auditorium_purpose (id),
    allowed_pool_id        INTEGER REFERENCES auditorium_pool (id),
    UNIQUE (discipline_course_id, position)
);
-- связующую таблицу для требований к оснащению
CREATE TABLE curriculum_slot_required_features
(
    slot_id    INTEGER NOT NULL REFERENCES curriculum_slot (id) ON DELETE CASCADE,
    feature_id INTEGER NOT NULL REFERENCES feature (id) ON DELETE CASCADE,
    PRIMARY KEY (slot_id, feature_id)
);
