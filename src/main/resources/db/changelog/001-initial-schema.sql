...
(
    id          INTEGER PRIMARY KEY,
    name        VARCHAR(255) NOT NULL,
    location_id BIGINT       NOT NULL REFERENCES location (id) ON DELETE RESTRICT, -- Нельзя удалить локацию, если в ней есть корпуса
    UNIQUE (location_id, name)                                                     -- Название корпуса должно быть уникальным в пределах одной локации
);
...
(
    ...
    building_id BIGINT       NOT NULL REFERENCES building (id) ON DELETE CASCADE, -- Аудитории удаляются вместе с корпусом
    UNIQUE (building_id, name)                                                    -- Номер аудитории уникален в пределах корпуса
);
...
CREATE TABLE groups
( -- Используем "groups" вместо "group", т.к. GROUP - ключевое слово SQL
    id                 INTEGER PRIMARY KEY,
    name               VARCHAR(255) NOT NULL UNIQUE,
