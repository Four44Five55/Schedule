...
function AppShell() {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'import':
        // Импорт чужой выгрузки. Период берёт из ОБЩЕГО контекста — того же, что раздел
        // «Расписание»: импорт пишет расписание в период, и пока период у него был свой,
        // записанное «нигде не находилось» (шапка показывала другой).
